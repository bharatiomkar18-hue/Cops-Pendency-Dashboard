...
const DEFAULT_REPO = "Cops-Pendency-Dashboard";
const DEFAULT_BRANCH = "main";
const STORE_PATH = "cops_shared_store.json";
...
function getConfig() {
  return {
    owner: process.env.COPS_GITHUB_OWNER || DEFAULT_OWNER,
    repo: process.env.COPS_GITHUB_REPO || DEFAULT_REPO,
    branch: process.env.COPS_GITHUB_BRANCH || DEFAULT_BRANCH,
    token: process.env.COPS_GITHUB_TOKEN || process.env.GITHUB_TOKEN || "",
    adminHash: process.env.COPS_ADMIN_PASSWORD_HASH || DEFAULT_ADMIN_PASSWORD_HASH
  };
}
...
async function writeStore(config, record) {
  if (!config.token) {
    throw new Error("Missing COPS_GITHUB_TOKEN. Add a GitHub token in Netlify environment variables.");
  }
